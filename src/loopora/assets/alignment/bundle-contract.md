...
- `inputs.evidence_query` accepts structured selector keys such as `archetypes`, `limit`, and `verifies`. Do not put explanatory prose, task instructions, or custom text keys inside `evidence_query`; put that prose in `spec.markdown`, role prompts, posture notes, or `workflow.collaboration_intent`.
...
- When that Guide was shaped by Inspector / Custom review, the following Builder must also name those review handoffs; the Guide handoff narrows the repair, but it is not a substitute for the review evidence chain.
...
- In a confirmed or compiling alignment phase, do not refuse to emit a bundle merely because the Agent cannot write files, cannot run backend validation itself, cannot inspect the full validator implementation, or doubts the response JSON schema. The Agent drafts the best complete YAML; Loopora writes, validates, and returns repair diagnostics.
...
- If project-local governance responsibilities are part of the agreement, they must reach closure, not just construction and review. Builder should read the markers, Inspector / Custom should verify relevant design or test obligations, and the finishing GateKeeper or equivalent closure surface must treat skipped project rules, skipped `design/README.md` / `design/` / `tests/` responsibilities, or missing expected validation as Weak, Unproven, Blocking, or fail-closed outcomes.
...
- Match the user's natural language in user-facing names and prose fields while preserving Loopora terms such as `spec`, `roles`, `workflow`, `bundle`, `Builder`, `Inspector`, `GateKeeper`, `Guide`, `Custom`, `workdir`, and `READY`. Generated visible names should carry task-language semantics instead of generic labels from another language. A role name such as a task phrase plus `Builder` / `Inspector` / `GateKeeper` still fails this rule when the task phrase, risk label, or responsibility qualifier is not in the user's display language.
