...
def _agent_run_sections(*, adapter: str, marker_source: str, context_bits: str) -> list[AdapterEntrySection]:
    return [
        ...
        AdapterEntrySection(
            "Work Panel Use",
            (
                "Read the top-level JSON `summary.agent_work_panel` first. In the main Agent session, report "
                ...
                "Bash/tool/JSON output alone does not count as the main-session work panel."
            ),
        ),
        AdapterEntrySection(
            "Role Dispatch",
            (
                "Invoke the exact host-native role agent named by `next_step.role_dispatch.target_agent`; copy "
                "`summary.next_role_dispatch_message` verbatim when present, and never submit inline work as if a role agent ran."
            ),
        ),
        AdapterEntrySection(
            "Result Template",
            (
                "In the main Agent session, open the active result template, preserve `loopora_host_dispatch`, fill only `result`, "
                "and submit the filled copy with the provided command."
            ),
        ),
        AdapterEntrySection(
            "Proof Boundary",
        ...
        AdapterEntrySection(
            "Failure Modes",
            (
                "If dispatch, schema, stale-step, or proof validation blocks submit, report the complete typed repair envelope "
                "and stop before inline fallback or shell-filtered summaries."
            ),
        ),
    ]
...
2. Read the returned JSON, even if the command exits nonzero. Do not pipe Loopora JSON, role wrappers, result wrappers, proof artifacts, or recovery payloads through `head`, `head -c`, `tail`, `sed`, Python string slicing, `python -c`, `jq`, `grep`, `wc`, `tee`, or other shell filters; read the complete compact JSON, or redirect the complete JSON to a workdir-local artifact first and parse a second copy from that preserved file, not a `/tmp` path. Never append a shell filter such as `2>&1 | head -200`, `| tee ... | wc -l`, or `; wc -c ...` to `loopora agent {adapter} run`, `next`, or `submit`. The same rule applies to any command output you later cite as proof: preserve complete output and an explicit exit code in the transcript or a cited artifact, then summarize it separately if it is long. If the payload has `ready: false` or `loop_recovery`, stop before dispatching any role agent:
   ...
   - `loop_recovery=finish_web_review` means the current `/loopora-plan` result needs Web review before `/loopora-run`; report `preview_url`, `requires_web_alignment`, `loopora_fit_contradiction`, `review_status`, `review_focus`, `run_blocked_until_web_review`, `after_review_cli_command_status`, `after_review_ready`, `after_web_review_cli_command`, and `after_review_command` instead of starting work. Do not run these commands until Web review completes.
