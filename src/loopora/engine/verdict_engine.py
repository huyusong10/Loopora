...
class VerdictEngine:
    def judge(self, contract: LoopContract, coverage: CoverageState, gatekeeper_result: StepResult | None = None) -> Verdict:
        ...
        if coverage.residual_risks and contract.residual_risk_policy == ResidualRiskPolicy.DISALLOW:
            return Verdict(
                run_id=coverage.run_id,
                status=VerdictStatus.CONTINUE_REQUIRED,
                source=source,
...
def _buckets_for(coverage: CoverageState) -> VerdictBuckets:
    proven: list[str] = []
    weak: list[str] = []
    unproven: list[str] = []
    blocking: list[str] = []
    ...
    return VerdictBuckets(
        ...
        residual_risk=coverage.residual_risks,
    )
