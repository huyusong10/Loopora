...
典型示例：退款自助流程、账单权限重构、跨服务支付回调问题、复杂迁移——这些任务的真正反馈在上线后、事故后、合规审查后才出现。
...
Agent 原生接入的能力契约刻意保持很小：

- 执行主体仍是当前宿主 Agent，工作目录也来自当前宿主上下文。Loopora 只拥有托管项目入口和 `.loopora/` 状态，不改写模型或 provider 路由、权限、审批模式、全局配置、宿主技能/插件、外部工具配置、凭据或环境密钥。
