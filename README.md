...
Web is the fuller observation and management surface. You can start a Loop from Agent, or open Web anytime to inspect and manage. Agent commands reuse a responding local Web service; otherwise they return a relative path and the foreground `loopora serve` command to run explicitly.
