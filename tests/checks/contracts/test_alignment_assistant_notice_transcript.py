...
def test_alignment_assistant_message_artifact_failure_does_not_block_durable_event(
    tmp_path: Path,
    monkeypatch,
) -> None:
    bundle_path = tmp_path / "align_1" / "artifacts" / "bundle.yml"
    ...
    def fail_transcript_open(path: Path, *args, **kwargs):
        if path == transcript_log:
    ...
    assert repo.session["transcript"][-1] == {
        "role": "assistant",
        "content": "Choose the next extraction target.",
        "created_at": "2026-05-29T00:01:00Z",
    }
    assert [event["event_type"] for event in repo.events] == ["alignment_message"]
