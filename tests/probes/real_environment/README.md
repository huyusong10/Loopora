...
- The host must create the candidate bundle from the conversation requirements; the harness must not prewrite it or embed a complete candidate YAML for the host to copy. The release prompt keeps this as `Use these requirements to author, not copy, the candidate`.
